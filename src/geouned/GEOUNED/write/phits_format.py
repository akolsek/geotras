    ...
    def write_phits_surfaces(self, surface):
        ...
        if PHITS_def:
            ...
            raise RuntimeError(
                ...
                "but cannot be written in PHITS format. The generated input would be invalid."
            )
        else:
            logger.warning(f"Unused surface {surface.Index} ({surface.Type}) cannot be written in PHITS format. Skipped.")
        return
